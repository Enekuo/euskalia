...
export default function PremiumGrammarCorrector() {
  ...
  const handleGenerate = async () => {
    ...
    if (!validNow) {
      setErrorMsg(
        tr("premiumGrammar.error_need_input", "Añade algo de texto, documentos o URLs antes de pedir la corrección.")
      );
      setLoading(false);
      return;
    }
    ...
    const systemBase =
      "Eres Euskalia Premium, un corrector gramatical y de estilo. " +
    ...
    try {
      ...
      if (!user) {
        throw new Error(tr("premiumGrammar.error_auth_required", "Necesitas iniciar sesión para usar Premium."));
      }
                  ...
                  {urlInputOpen && (
                    <div className="mb-4 rounded-xl border border-slate-300 p-3 bg-white">
                      <textarea
                        ...
                        placeholder={tr(
                          "premiumGrammar.paste_urls_placeholder",
