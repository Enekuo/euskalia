...
export default function PremiumParaphraser() {
  ...
  // ✅✅✅ PREMIUM LIMITS (patrón único) -> NO GUARDAR TEXTO TRADUCIDO EN STATE
  ...
  // Idioma de salida
  const [outputLang, setOutputLang] = useState("eus"); // eus | es | en | fr
  ...
  // ===== Labels (claves NUEVAS PREMIUM) =====
  const labelSources = tr("premiumParaphraser.sources_title", "Fuentes");
  const labelTabText = tr("premiumParaphraser.tab_text", "Texto");
  const labelTabDocument = tr("premiumParaphraser.tab_document", "Documento");
  const labelTabUrl = tr("premiumParaphraser.tab_url", "URL");
  const labelEnterText = tr("premiumParaphraser.enter_text_placeholder", "Escribe o pega tu texto aquí…");

  const labelChooseFileTitle = tr("premiumParaphraser.pick_file_title", "Elige tu archivo o carpeta");
  const labelAcceptedFormats = tr(
    "premiumParaphraser.accepted_formats",
    "Puedes añadir archivos PDF, texto copiado, enlaces web…"
  );
  const labelFolderHint = tr("premiumParaphraser.folder_hint", "Aquí aparecerán tus textos o documentos subidos.");

  const labelPasteUrls = tr("premiumParaphraser.paste_urls_label", "Pegar URLs*");
  const labelAddUrl = tr("premiumParaphraser.add_urls_button", "Añadir URLs");
  const labelSaveUrls = tr("premiumParaphraser.save_urls_button", "Guardar");
  const labelCancel = tr("premiumParaphraser.cancel_button", "Cancelar");
  const labelUrlsNoteVisible = tr(
    "premiumParaphraser.urls_note_visible",
    "Solo se importará el texto visible del sitio web."
  );
  const labelUrlsNotePaywalled = tr("premiumParaphraser.urls_note_paywalled", "No se admiten artículos de pago.");
  const labelRemove = tr("premiumParaphraser.remove_button", "Quitar");
  ...
  const labelGenerateFromSources = tr("premiumParaphraser.generate_button", "Crear parafraseo");
  const labelHelpRight = tr(
    "premiumParaphraser.help_right",
    'Selecciona una fuente (texto, documentos o URLs) y pulsa "Crear parafraseo".'
  );

  const labelSaveButton = tr("premiumParaphraser.save_to_library_button", "Guardar");
  const librarySavedMessage = tr("premiumParaphraser.saved_to_library", "Guardado en biblioteca");

  const ariaCopyResult = tr("premiumParaphraser.copy_result_aria", "Copiar resultado");
  const ariaDeleteInput = tr("premiumParaphraser.delete_input_aria", "Eliminar texto de entrada y resultado");
  const titleCopyResult = tr("premiumParaphraser.copy_result_title", "Copiar resultado");
  const titleDeleteInput = tr("premiumParaphraser.delete_input_title", "Eliminar texto de entrada y resultado");

  const labelUrlTextareaPlaceholder = tr(
    "premiumParaphraser.urls_textarea_placeholder",
    "Introduce aquí una o más URLs (separadas por línea)"
  );

  const labelDownload = tr("premiumParaphraser.download", "Descargar");
  const labelCopy = tr("premiumParaphraser.copy", "Copiar");
  const labelCopied = tr("premiumParaphraser.copied", "Copiado");

  // Ayuda izquierda
  const leftTitle = tr("premiumParaphraser.left_title", "Aquí aparecerán tus textos o documentos subidos.");
  const leftBody = tr("premiumParaphraser.left_body", "Puedes añadir archivos PDF, texto copiado, enlaces web…");
  ...
  const modeLabels = {
    neutral: tr("premiumParaphraser.mode_neutral", "Neutral"),
    informal: tr("premiumParaphraser.mode_informal", "Informal"),
    professional: tr("premiumParaphraser.mode_professional", "Profesional"),
    academic: tr("premiumParaphraser.mode_academic", "Académico"),
    fluent: tr("premiumParaphraser.mode_fluent", "Fluido"),
    simplified: tr("premiumParaphraser.mode_simplified", "Simplificado"),
    creative: tr("premiumParaphraser.mode_creative", "Creativo"),
  };
  ...
  // URLs / docs / modo / idioma salida cambian => limpia derecha
  useEffect(() => {
    clearRight();
  ...
  const handleSaveToLibrary = () => {
    ...
    addLibraryDoc({
      kind: "paraphraser",
      title: titleFromText || tr("premiumParaphraser.default_title", "Parafraseo"),
      content: result,
      createdAt,
      createdAtLabel,
    });
  ...
  const handleGenerate = async () => {
    ...
    if (!validNow) {
      setErrorMsg(
        tr(
          "premiumParaphraser.error_need_input",
...
`.trim();
    ...
    try {
      ...
      if (!user) {
        throw new Error(tr("premiumParaphraser.error_auth_required", "Necesitas iniciar sesión para usar Premium."));
      }
      ...
      // ✅✅✅ PREMIUM LIMITS 429 (patrón único)
      if (res.status === 429) {
        let data = null;
        try {
          data = await res.json();
      ...
      if (!rawText) throw new Error(tr("premiumParaphraser.error_no_text", "No se recibió texto de la API."));
      ...
      setErrorMsg(err.message || tr("premiumParaphraser.error_generic", "Error creando el parafraseo."));
    } finally {
      setLoading(false);
    }
  };
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      ...
                      aria-label={tr("premiumParaphraser.output_language_aria", "Idioma de salida")}
                    >
